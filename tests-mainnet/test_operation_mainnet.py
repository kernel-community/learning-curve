...
def test_full_redeem(
    ...
    tx = deschool.batchDeposit(sender=kernelTreasury)
    ...
    for n, learner in enumerate(learners):
        ...
        print("redeemable DAI Balance of Creator: " +
              str(deschool.getYieldRewards(kernelTreasury.address, sender=kernelTreasury))
              )
...
def test_full_mint(
    ...
    for learner in reversed(learners):
        ...
        learning_curve.approve(
            ...
            lc_balance_before,
            sender=learner)
        tx = learning_curve.burn(lc_balance_before, sender=learner)
