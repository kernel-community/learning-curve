...
def test_register_permit(contracts_with_courses, learners, token, deployer):
    ...
    token.transfer(holder, constants_unit.STAKE, sender=deployer)
...
def test_create_malicious_courses(contracts, hackerman):
    deschool, learning_curve = contracts
    with ape.reverts("createCourse: stake must be greater than 0"):
    ...
    with ape.reverts("createCourse: duration must be greater than 0"):
        deschool.createCourse(
            constants_unit.STAKE,
            0,
            constants_unit.URL,
...
def test_register_malicious(contracts_with_courses, token, deployer, hackerman):
    deschool, learning_curve = contracts_with_courses
    with ape.reverts():
        deschool.register(0, sender=hackerman)

    token.transfer(hackerman, constants_unit.STAKE, sender=deployer)
    token.approve(deschool, constants_unit.STAKE, sender=hackerman)
    with ape.reverts("register: courseId does not exist"):
        deschool.register(999, sender=hackerman)

    with ape.reverts("register: courseId does not exist"):
        deschool.register(deschool.getNextCourseId(), sender=hackerman)

    deschool.register(0, sender=hackerman)
    token.transfer(hackerman, constants_unit.STAKE, sender=deployer)
    token.approve(deschool, constants_unit.STAKE, sender=hackerman)
    with ape.reverts("register: already registered"):
        deschool.register(0, sender=hackerman)
...
def test_mint_malicious(contracts_with_learners, hackerman, learners):
    deschool, learning_curve = contracts_with_learners
    with ape.reverts("mint: not a learner on this course"):
        deschool.mint(0, sender=hackerman)
    ape.chain.mine(constants_unit.COURSE_RUNNING)
    with ape.reverts("mint: not yet eligible - wait for the full course duration to pass"):
        deschool.mint(0, sender=learners[0])
...
def test_redeem_malicious(hackerman, contracts_with_learners, learners):
    deschool, learning_curve = contracts_with_learners
    with ape.reverts("redeem: not a learner on this course"):
        deschool.redeem(0, sender=hackerman)
    ape.chain.mine(constants_unit.COURSE_RUNNING)
    with ape.reverts("redeem: not yet eligible - wait for the full course duration to pass"):
        deschool.redeem(0, sender=learners[0])
...
def test_verify_malicious(contracts_with_learners, hackerman, learners):
    deschool, learning_curve = contracts_with_learners
    learner = learners[0]
    with ape.reverts("verify: courseId does not exist"):
        deschool.verify(learner, 999, sender=hackerman)
    with ape.reverts("verify: courseId does not exist"):
        deschool.verify(learner, deschool.getNextCourseId(), sender=hackerman)
    with ape.reverts("verify: not registered to this course"):
        deschool.verify(hackerman, 0, sender=hackerman)
...
def test_mint_lc_not_initialised(token, deployer, steward, learners):
    ...
    deschool = project.DeSchool.deploy(
        token.address,
    ...
    for n, learner in enumerate(learners):
        ...
        ape.chain.mine(constants_unit.DURATION)
        with ape.reverts("!initialised"):
            deschool.mint(0, sender=learner)
