...
def test_flash_behaviour(token, deployer, hackerman, contracts, learners):
    ...
    token.approve(learning_curve, constants_unit.MALICIOUS_AMOUNT, sender=hackerman)
    learning_curve.mint(constants_unit.MALICIOUS_AMOUNT, sender=hackerman)
    ...
    for learner in learners:
        ...
        token.approve(learning_curve, constants_unit.MINT_AMOUNT, sender=learner)
        learning_curve.mint(constants_unit.MINT_AMOUNT, sender=learner)
    n = 0
    ...
    tx = learning_curve.burn(learning_curve.balanceOf(hackerman), sender=hackerman)
...
def test_init(token, deployer):
    ...
    assert token.balanceOf(learning_curve) == 10**18
    assert learning_curve.totalSupply() == 10001*10**18
    assert learning_curve.reserveBalance() == 1*10**18


def test_init_malicious(token, deployer, hackerman):
    learning_curve = project.LearningCurve.deploy(token.address, sender=deployer)
    with ape.reverts():
        learning_curve.initialise(sender=hackerman)
    with ape.reverts():
        learning_curve.initialise(sender=deployer)
    token.approve(learning_curve, 1*10**18, sender=deployer)
    learning_curve.initialise(sender=deployer)
...
def test_mint(learners, token, deployer, contracts):
    ...
    for n, learner in enumerate(learners):
        ...
        learner_before_dai_bal = token.balanceOf(learner)
        learner_before_lc_bal = learning_curve.balanceOf(learner)
        numerator = float((learning_curve.reserveBalance() / 1*10**18 + constants_unit.MINT_AMOUNT / 1*10**18))
        predicted_mint = float(constants_unit.K * ln(numerator / (learning_curve.reserveBalance() / 1*10**18))) * 1*10**18
...
def test_mint_permit(token, deployer, contracts):
    ...
    token.transfer(holder, constants_unit.MINT_AMOUNT, sender=deployer)
    ...
    assert abs(predicted_mint - learning_curve.getMintableForReserveAmount(constants_unit.MINT_AMOUNT)) < \
               constants_unit.ACCURACY
    tx = learning_curve.permitAndMint(constants_unit.MINT_AMOUNT, 0, 0, signed.v, signed.r, signed.s, sender=holder)
    print(token.balanceOf(learning_curve.address))
...
def test_burn(learners, token, deployer, contracts):
    ...
    for learner in reversed(learners):
        ...
        learner_before_dai_bal = token.balanceOf(learner)
        learner_before_lc_bal = learning_curve.balanceOf(learner)
        numerator = float((learning_curve.reserveBalance() / 1*10**18))
